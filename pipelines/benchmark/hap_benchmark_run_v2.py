...
def germline_variant_calling(gatk_dir, marked_BQSR_bam,
    ...
    if os.path.basename(exon_interval) != 'all':
...
def vardict_call(vardict_dir, total_ref_fa_file, outputdir,sample, bam, exome_target_bed, af):
    ...
    if os.path.basename(exon_interval) != 'all':
        ...
        command = '{0} -G {1} -N {2} -b {3} -f {4} -c 1 -S 2 -E 3 > {5}'.format(
        vardict_dir,total_ref_fa_file, bam1.rstrip('.bam'), bam1, af ,tmp)
    print(command)
    os.system(command)
...
def strelka2_call(strelka2_dir,
    ...
    if os.path.basename(exon_interval) != 'all':
        ...
        command3='python {0} --bam {1} --exome --referenceFasta {2} --runDir {3}'.format(
            strelka2_dir, bam, total_ref_chrom_fa_file, outputdir)
...
def main():
    ...
    if yaml_file != 'null' and 'exon_interval' in file_yaml.keys():
        exon_interval = args.datasets_dir + '/' + file_yaml['exon_interval']
    else:
        exon_interval = args.datasets_dir + '/' + args.exon_interval
    ...
    #----
    bam_to_variant  = file_yaml['vready_bam']
    
    #GATK-without BQSR
      #time cost
    time_start = time.time()
    germline_vc_dir1= out_dir + '/'+ 'gatk_without_bqsr'
    if '' in file_yaml['softs']:
