...
option_list <- list(
  ...
  make_option(c("-m", "--mt_depth"), type="character", 
              help="Input mt depth of target region"),
...
if(TRUE){
 ...
 for(i in 1:nrow(files)){
  ...
  exon_statis_mes<-cbind(Library_name=c("Length of target region",
                                        ...
                                        "Maximun BD","% of target bases with BD >= 50x",
...
colnames(exon_statis_mtdp)<-colnames(qcs)
# trim
...
# bases dp in target region
exon_statis_mes<-exon_statis_mes[-c(10:13),]
data<-rbind(data, exon_statis_mes)
# bases MT dp in target region
data<-rbind(data, exon_statis_mtdp)

colnames(data)<-colnames(qcs)
data<- as.matrix(data)
write.table(data, file=opts$output, append = T, quote = F, sep="\t", eol = "\n", na = "NA", dec = ".", row.names = F, col.names = T)
write.table(primersfilecount, file=gsub('.txt','_primercount.txt',opts$output), 
            append = T, quote = F, sep="\t", eol = "\n", na = "NA", dec = ".", row.names = F, col.names = T)
write.table(primersfilepre, file=gsub('.txt','_primer.txt',opts$output), 
            append = T, quote = F, sep="\t", eol = "\n", na = "NA", dec = ".", row.names = F, col.names = T)