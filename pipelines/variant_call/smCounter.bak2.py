...
#-------------------------------------------------------------------------------------
# function to get the model seq
#-------------------------------------------------------------------------------------
def get_mode_seq(list1):
    l1 = len(list1)
    ...
    for m in range(l2):
        ...
        for n in range(l1):
            ...
            if list2[m] in tuple(list1)[start:stop]:
                ...
                x.add(a)
                start = start + 1
        seq.append(str(list2[m]))
        num.append(len(x))
        indexsets.append(x)
    return seq[num.index(max(num))], indexsets[num.index(max(num))]
#-------------------------------------------------------------------------------------
...
def vc(bamFile, chrom, pos, minBQ, minMQ, mtDepth, rpb, hpLen, mismatchThr, mtDrop, maxMT, primerDist, refGenome):
   ...
   # get mate reads
   matereads = defaultdict(lambda: defaultdict(list))
      ...
      for pileupRead in read.pileups:
         ...
         if incCond:
            if readid not in bcDict[BC]:
               ...
               matereads[BC][readid].append(materead)
   ...
   # check the meta reads of the the same umi
   bcKeys = bcDict.keys()
   for bc in bcKeys:
      if len(bcDict[bc]) is 1:
         continue
      posmeta=[]
      for readid in bcDict[bc]:
         mate = matereads[bc][readid]
         for read in mate:
             posmeta.append(read.pos)
      poss, indexs = get_mode_seq(posmeta)
      bcreadsid = list(bcDict[bc].keys())
      if len(indexs) < len(bcreadsid):
         for index in range(0, len(bcreadsid)):
            if index not in indexs:
               del bcDict[bc][bcreadsid[index]]
...
def main(args):
   ...
   # read bam to hashtable to get the mate
   #readshash = U.HashTable()
   bf = pysam.AlignmentFile(args.bamFile, 'rb')
   for r in bf:
       qname = r.query_name
       qnameSplit = qname.split(":")
       readid = ':'.join(qnameSplit[:-2])
       bcids = qnameSplit[-2].split("-")
       readid = readid + bcids[1]
       readshash.add(readid, r)
       #readshash[readid] = r
   bf.close()
